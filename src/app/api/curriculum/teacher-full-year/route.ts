...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const day of dayNames) {
      ...
      for (const s of daySchedules) {
        ...
        weeklyCalendar[day].push({
          ...
          endTime: s.endTime,
          scheduledTopic: s.topic || plan?.topic || `${s.subject} Lesson`,
          lessonPlan: plan ? {
            id: plan.id,
            topic: plan.topic,
    ...
    const assignedGrades = [...new Set(teacher.schedules.map(s => s.grade))].sort();
    const assignedSubjects = [...new Set(teacher.schedules.map(s => s.subject))].sort();
    const assignedSections = [...new Set(teacher.schedules.map(s => `${s.grade} ${s.section}`))].sort();
